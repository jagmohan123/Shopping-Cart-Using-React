...
const Product = ({ item }) => {
  ...
  hover:scale-110 transition duration-300 ease-in gap-2 p-4 mt-10 ml-8 rounded-xl border-[3px] hover:shadow-2xl">
    ...
    <div>
      ...
      <p className="w-30 text-gray-400 font-normal text-[20px] text-left">{item.description.split(" ").slice(0, 10).join(" ")}</p>
    </div>
    <div className="h-[180px]">
      <img src={item.image} alt="loading" loading="lazy" className="h-full w-full " />
    </div>
    ...
    eske leaye we need cart */}

      {
        ...
        cart.some((Singleitem) => Singleitem.id === item.id) ?
          (<button
